...
@note:  This is an Ant Colony Optimization Algorithm program. It generates and solves random networks and
        saves them in a directory for other programs to work with the data.
...
class ACO():
    ...
    def __init__(self, nodes, iterations, learning_rate, path):
        self.nodes = nodes
...
data_path = "C:/ENTER_THE_PATH_HERE" # path where the data is saved in text files
amount_of_data_files = 10 # number of text files to be created

# clear old data
clear_data(data_path)
# create new instance of the ACO
aco = ACO(nodes, training_iterations, learning_rate, data_path)
...
while progress < amount_of_data_files:
    ...
    coordinates = aco.init_coordinates() # save the randomly created network in a variable
